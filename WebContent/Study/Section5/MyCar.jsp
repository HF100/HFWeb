	...
	<%
		...
		try {
			...
			if (request.getParameter("Goods_id") == null
					|| request.getParameter("Goods_id").trim().length() == 0) {
				//noting to do
			} else {
				int Goods_id = Integer.parseInt(request
						.getParameter("Goods_id"));
				String stringSqlString = "SELECT * FROM Goods where Goods_id="
						+ Goods_id;
				//设置SQL语句
				...
				out.println("<input type=\"hidden\" name=Goods_id value="
						+ Goods_id + ">");
			...
			Hashtable<String,Integer> list = mycar.listMyGoods();
			...
			while (enum1.hasMoreElements()) {
				...
				out.println("<td>" + rs.getDouble("Goods_money")
						* Float.parseFloat(list.get(goods).toString())
						+ "</td>");
				out.println("<td><a href='DeleteGoods.jsp?Goods_id="
						+ goods + "'>从购物车中删除</td>");
				...
				out.println("<input type=\"submit\" value=\"提交\"></td></form>");
				out.println("</tr>");
				all_price = all_price + rs.getDouble("Goods_money")
						* Float.parseFloat(list.get(goods).toString());
			}
